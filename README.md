...
| `KMC_DEVBOX_IMAGE`         | (first ubuntu golden)   | Golden image for Dev Boxes (`name` or `namespace/name`)                       |
| `KMC_SNAPSHOT_JOB_IMAGE`   | `ghcr.io/ianunruh/kmc:latest` | Container image for per-VM snapshot CronJobs (`scripts/snapshot-run.ts`) |

## Features (MVP)

- **Dev Boxes** — opinionated VMs (`kmc.ianunruh.com/resource=devbox`) from hardcoded templates (`ubuntu`, `ubuntu-docker`, `ubuntu-docker-code`). Create stamps owner, composes cloud-init in a Secret (guest-agent, git, optional docker / public repo clone / code-server), and creates a MetalLB LoadBalancer (`<vm>-ssh`, internal pool from `clusters.yaml` `devbox.metallb`) whose VIP is the SSH / VS Code / Cursor address. Reachability assumes **KCloud Tailscale**. `ubuntu-docker-code` also creates an HTTPRoute + Envoy `SecurityPolicy`. OIDC is a per-box Dex `OAuth2Client` CR (in the Dex namespace) plus a Secret in the VM namespace — callers need create/delete on `oauth2clients.dex.coreos.com` there (`deploy/controller/rbac/user-devbox-oidc-example.yaml`). Access tab is on every VM. Image default: `KMC_DEVBOX_IMAGE`, else first ready golden whose name contains `ubuntu`.
- **Virtual machines** — list, create, detail, edit (labels always; size / preference / run strategy when stopped), stop/start/restart/pause/unpause/delete, **snapshots / in-place restore** (KubeVirt `VirtualMachineSnapshot` / `VirtualMachineRestore` on the VM detail page), **snapshot schedules** (per-VM CronJob + retention on the Storage tab), **serial console** (boot/debug) and **SSH terminal** (browser shell via platform key + port-forward). Console-created VMs stamp `kmc.ianunruh.com/owner` from the GitHub session (claimable later if missing).
